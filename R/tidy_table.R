...
#' (HD2 <- copy(HD)[grepl("^No\\.", Var), Var := "N"][...col... == 1, ...LONG... := "Lbound"][])
#' tidy_data(x[5:6], HD2)
...
tidy_table <- function(DT, header_rows, long_cols = NULL, wide_cols = NULL,
  ...
  .tidyHeader <- tidy_header(
    DT_header,
    long_cols = long_cols, ## quote
    wide_cols = wide_cols, ## quote
    wide_names = wide_names,
    wide_fill = wide_fill,
    wide_split = wide_split,
    cols_delete = cols_delete, ## quote
    cols_keepnames = cols_keepnames, ## quote
...
# Columns classification (into long or wide)
  ...
  cols_long0 <- cnames_q(DT, long_cols)
  cols_wide0 <- cnames_q(DT, wide_cols)
  ...
  if (is_null_delete) {
    ...
    cols_delete <- cnames_q(DT, cols_delete)
  }
